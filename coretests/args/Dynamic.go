...
func (it *Dynamic) HasFirst() bool { return it.Params.HasFirst() }

func (it *Dynamic) GetByIndex(index int) any { return it.Params.GetByIndex(index) }
func (it *Dynamic) HasFunc() bool { return it.Params.HasFunc() }
func (it *Dynamic) GetFuncName() string { return it.Params.GetFuncName() }

func (it *Dynamic) Invoke(args ...any) (results []any, processingErr error) {
...
func (it *Dynamic) InvokeWithValidArgs() (results []any, processingErr error) {
	return it.Params.InvokeWithValidArgs()
}

func (it *Dynamic) InvokeArgs(names ...string) (results []any, processingErr error) {
...
func (it *Dynamic) ThirdItem() any  { return it.Params.ThirdItem() }
func (it *Dynamic) FourthItem() any { return it.Params.FourthItem() }
func (it *Dynamic) FifthItem() any  { return it.Params.FifthItem() }
func (it *Dynamic) SixthItem() any  { return it.Params.SixthItem() }
func (it *Dynamic) Expected() any   { return it.Expect }
...
func (it *Dynamic) HasDefined(name string) bool {
	...
	return has && reflectinternal.Is.Defined(item)
}
...
func (it *Dynamic) HasDefinedAll(names ...string) bool {
	if it == nil || len(names) == 0 { return false }
	for _, name := range names {
		if it.IsKeyInvalid(name) { return false }
	}
	return true
}
...
func (it *Dynamic) IsKeyInvalid(name string) bool {
	if it == nil { return false }
	item, has := it.Params[name]
	return !has || reflectinternal.Is.Null(item)
}
...
func (it *Dynamic) Actual() any  { return it.GetDirectLower("actual") }
func (it *Dynamic) Arrange() any { return it.GetDirectLower("arrange") }
...
func (it *Dynamic) ValidArgs() []any {
	...
	for _, key := range keys {
		val := it.Params[key]
		if isDefined(val) && isNotFunc(val) { args = append(args, val) }
	}
	return args
}
...
func (it *Dynamic) String() string {
	...
	toFinalString := fmt.Sprintf(selfToStringFmt, "Dynamic", strings.Join(args, constants.CommaSpace))
	return it.toString.GetSetOnce(toFinalString)
}

func (it Dynamic) AsArgsMapper() ArgsMapper                         { return &it }
func (it Dynamic) AsArgFuncNameContractsBinder() ArgFuncNameContractsBinder { return &it }
func (it Dynamic) AsArgBaseContractsBinder() ArgBaseContractsBinder         { return &it }
