...
var hasIndexTestCases = []coretestcases.CaseV1{
	{
		...
		ArrangeInput:  args.Map{"when": "given matching index", "indexes": []int{1, 3, 5}, "current": 3},
		ExpectedInput: args.Map{"result": true},
	},
	{
		Title:         "HasIndex returns false when index missing",
		ArrangeInput:  args.Map{"when": "given non-matching index", "indexes": []int{1, 3, 5}, "current": 4},
...
var lastIndexTestCases = []coretestcases.CaseV1{
	{
		...
		ArrangeInput:  args.Map{"when": "given length 5", "length": 5},
		ExpectedInput: args.Map{"result": 4},
	},
	{
		Title:         "LastIndex of length 1 is 0",
		ArrangeInput:  args.Map{"when": "given length 1", "length": 1},
...
var isWithinIndexRangeTestCases = []coretestcases.CaseV1{
	{
		...
		ArrangeInput:  args.Map{"when": "given index within range", "index": 2, "length": 5},
		ExpectedInput: args.Map{"result": true},
	},
	{
		Title:         "IsWithinIndexRange false for out-of-range index",
		ArrangeInput:  args.Map{"when": "given index beyond range", "index": 5, "length": 5},
		ExpectedInput: args.Map{"result": false},
	},
}
