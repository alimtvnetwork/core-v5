...
func makeTypedWrapper(name, id string, data testUser) *corepayload.TypedPayloadWrapper[testUser] {
	tw, err := corepayload.NewTypedPayloadWrapperFrom[testUser](name, id, "testUser", data)
	if err != nil {
		panic(err)
	}
	return tw
...
func makeCollection() *corepayload.TypedPayloadCollection[testUser] {
	col := corepayload.NewTypedPayloadCollection[testUser](3)
	col.Add(makeTypedWrapper("user", "1", testUser{Name: "Alice", Email: "a@a.com", Age: 30}))
	col.Add(makeTypedWrapper("user", "2", testUser{Name: "Bob", Email: "b@b.com", Age: 25}))
	col.Add(makeTypedWrapper("user", "3", testUser{Name: "Carol", Email: "c@c.com", Age: 35}))
	return col
}
