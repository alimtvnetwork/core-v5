...
func TestResult_IsErrorEqual(t *testing.T) {
	r := &corejson.Result{Error: errors.corejson.New("test")}
	if !r.IsErrorEqual(errors.corejson.New("test")) { t.Fatal("expected true") }
}
...
func TestResult_String_WithError(t *testing.T) {
	r := corejson.Result{Error: errors.corejson.New("test"), Bytes: []byte(`"x"`), TypeName: "test"}
	s := r.String()
	if s == "" { t.Fatal("expected non-empty") }
}
...
func TestResult_MeaningfulError_WithError(t *testing.T) {
	r := &corejson.Result{Error: errors.corejson.New("fail"), Bytes: []byte(`"x"`)}
	if r.MeaningfulError() == nil { t.Fatal("expected error") }
}
...
func TestResult_InjectInto(t *testing.T) {
	r := corejson.NewResult.Any([]string{"a"})
...
func TestResult_Deserialize(t *testing.T) {
	r := corejson.NewResult.Any([]string{"a", "b"})
	var target []string
...
func TestResult_UnmarshalResult(t *testing.T) {
	r := corejson.NewResult.Any(corejson.Result{TypeName: "test"})
...
func TestSerialize_Bytes(t *testing.T) {
	b, err := corejson.Serialize.Bytes("hello")
	if err != nil || len(b) == 0 { t.Fatal("unexpected") }
}

func TestSerialize_String(t *testing.T) {
	s, err := corejson.Serialize.String("hello")
	if err != nil || s == "" { t.Fatal("unexpected") }
}
...
func TestDeserialize_UsingBytes(t *testing.T) {
	...
	var target string
	err := corejson.Deserialize.UsingBytes(b, &target)
	if err != nil || target != "hello" { t.Fatal("unexpected") }
}

func TestDeserialize_UsingString(t *testing.T) {
	var target string
	err := corejson.Deserialize.UsingString(`"hello"`, &target)
	if err != nil || target != "hello" { t.Fatal("unexpected") }
}
...
func TestAnyTo_PrettyString(t *testing.T) {
	s := corejson.AnyTo.PrettyString(map[string]string{"a": "1"})
	if s == "" { t.Fatal("expected non-empty") }
}
